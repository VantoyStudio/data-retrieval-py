...
def main():
    ...
    dataPath = os.path.join(path, "data" , current_dt.strftime("%Y-%m-%d-%H-%M-%S")) 
    
    # make directories
    os.mkdir(dataPath)

    crop_path = os.path.join(dataPath, "cropped")
    alt_path = os.path.join(dataPath, "altered")
    orig_path = os.path.join(dataPath, "original")
    ...
    while(cap.isOpened()):
        current = time.time()
        ret, frame = cap.read()
        filename = current_dt.strftime("%Y-%m-%d-%H-%M-%S")+str(count)+".png"
        # filePath = os.path.join(dataPath, filename)

         
        filePath_alt =  os.path.join(alt_path, filename)
        filePath_orig =  os.path.join(orig_path, filename)
        ...
        if ret == True:
            ...
            cv2.imwrite(filePath_orig , frame)
            ...
            for img in cropped:
                ...
                inner_count += 1
                fileName = current_dt.strftime("%Y-%m-%d-%H-%M-%S")+str(count)+"-"+str(inner_count)+".png"
                filePath_crop = os.path.join(crop_path, fileName)
                #if abspath is not None:
                try:
                    cv2.imwrite(filePath_crop, img)
    ...
    try:
        ...
        print("Directory size: "+str(math.floor(get_size(os.path.join(os.getcwd(), "data")) / (1024*1024)))+" MB")
